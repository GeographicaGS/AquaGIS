...
CREATE OR REPLACE FUNCTION urbo_aq_cons_agg_forecast_hourly(
  ...
  BEGIN
    ...
    _t_const_ag_fc := urbo_get_table_name(id_scope, 'aq_cons_const_agg_forecast_hour');

    _t_plot_ld  := urbo_get_table_name(id_scope, 'aq_cons_plot', FALSE, TRUE);
    _t_plot_ag_fc := urbo_get_table_name(id_scope, 'aq_cons_plot_agg_forecast_hour');

    _t_sector_ag_fc := urbo_get_table_name(id_scope, 'aq_cons_sector_agg_forecast_hour');
    ...
    _q := format('
      ...
      ON CONFLICT (id_entity, "TimeInstant")
        DO UPDATE SET
          ...
          pressure_forecast = EXCLUDED.pressure_forecast;
      ',
      _t_const_ag_fc, moment, _t_aux_ft, moment, moment,

      _t_plot_ag_fc, moment, _t_const_ag_fc, moment, moment, _t_const_ld,

      _t_sector_ag_fc, moment, _t_plot_ag_fc, moment, moment, _t_plot_ld
    );
