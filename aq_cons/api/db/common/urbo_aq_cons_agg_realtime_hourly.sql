...
CREATE OR REPLACE FUNCTION urbo_aq_cons_agg_realtime_hourly(
  ...
  BEGIN
    ...
    _t_const_ag_rt := urbo_get_table_name(id_scope, 'aq_cons_const_agg_realtime_hour');

    _t_plot_ld  := urbo_get_table_name(id_scope, 'aq_cons_plot', FALSE, TRUE);
    _t_plot_ag_rt := urbo_get_table_name(id_scope, 'aq_cons_plot_agg_realtime_hour');

    _t_sector_ag_rt := urbo_get_table_name(id_scope, 'aq_cons_sector_agg_realtime_hour');
    ...
    _q := format('
      ...
      ON CONFLICT (id_entity, "TimeInstant")
          DO UPDATE SET
            ...
            pressure_agg = EXCLUDED.pressure_agg;
      ',
      _t_const_ag_rt, moment, _t_const_ms, moment, moment,

      _t_plot_ag_rt, moment, _t_const_ag_rt, moment, moment, _t_const_ld,

      _t_sector_ag_rt, moment, _t_plot_ag_rt, moment, moment, _t_plot_ld, _t_aux_lk,
      moment, moment
    );
