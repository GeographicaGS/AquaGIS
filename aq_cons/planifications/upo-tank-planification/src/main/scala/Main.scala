...
object Main {
  ...
  /**
    * Función principal del program. No recibe parámetros.
    *
  ...
  def main(args: Array[String]): Unit = {
    ...
    val hora_planificador = Time_utils.addMinutesToString(init_time, 0)
    ...
    // La llave del while termina en línea 182
    /*
        val init_time = "2018-09-24 23:00:00" // System.getTime()
        Time_utils.syncTimesTo(init_time)
        while (Time_utils.currentTimestamp != "2018-11-08 09:00:00") {
        Time_utils.syncTimesTo(Time_utils.addMinutesToString(Time_utils.currentTimestamp, 60))
    */



    Logger.getGlobal.info(s"currentTimestamp: ${Time_utils.currentTimestamp}")
    ...
    if (Time_utils.currentTimestamp.endsWith(" 00:00:00")) {
      Logger.getGlobal.info(s"INICIADO PLANIFICADOR")
      ...
      tanks foreach { tank =>
        val startLevel_m3: (String, Double) = tankDAO.startLevel_m3_ByTankId(tank.id_entity)
        val sectorsIdList: Seq[String] = tankDAO.listSectorsByTankId(tank.id_entity)
        val forecast: Seq[(String, Double)] = tankDAO.loadAggregatedForecastFromSectorsByTankId(tank.id_entity)
      ...
      tanks foreach { tank =>
        ...
        if (tank.currentLevel_m3._2 < tank.min_level * tank.capacity / 100.0) {
          ...
          Logger.getGlobal.info(s"DETECTADA EMERGENCIA EN LA RUTINA DEL PLANIFICADOR")
          ...
          val lastEmergencies: Seq[(String, Boolean)] = tankDAO.loadPumpEmergenciesOfYesterdayByTankId(tank.id_entity)
          ...
          if (lastEmergency._1 != "" && lastEmergency._2 == true) {
            tankDAO.saveEmergency(tank.id_entity, Time_utils.currentTimestamp, activated = false)
          }
        }
      ...
      Logger.getGlobal.info(s"INICIADO MOTOR DE EMERGENCIA/MANTENIMIENTO")
      ...
      tanks foreach { tank =>
        ...
        val lastLevel_m3: (String, Double) = tankDAO.loadLastLevelByTankId(tank.id_entity, tank.start_level_m3._2, table = "aq_plan_tank_pump_opt")
        tank.setCurrentLevel_m3(lastLevel_m3)
