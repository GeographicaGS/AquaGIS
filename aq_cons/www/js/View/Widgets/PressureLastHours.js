...
App.View.Widgets.Aq_cons.PressureLastHours = App.View.Widgets.Base.extend({

  initialize: function(options) {
    var that = this;
    options = _.defaults(options,{
      title: 'Presión: Últimas 24 horas',
      timeMode:'now',
      id_category: 'aq_cons',
      publishable: false,
      classname: 'App.View.Widgets.Aq_cons.PressureLastHours',
    });
    App.View.Widgets.Base.prototype.initialize.call(this,options);
    this.searchParams = function(agg) {
      return {
        "agg": ["AVG","MIN","MAX"],
        "vars": ["aq_cons.sector.pressure","aq_cons.sector.pressure","aq_cons.sector.pressure"],
        "time": {
    ...
    this._model.parse = function(data) {
      ...
      if (data && data.length > 0) {
        _.each(data[0].data['aq_cons.sector.pressure'], function(d) {
